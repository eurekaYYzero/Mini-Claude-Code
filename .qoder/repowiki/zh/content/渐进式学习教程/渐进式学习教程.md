...
- [src/s12/index.ts](file://src/s12/index.ts)
- [src/s12/index.py](file://src/s12/index.py)
- [src/s12/package.json](file://src/s12/package.json)
</cite>

## 更新摘要
**变更内容**
- 从简单的todo管理转向完整的生产级任务管理框架
- 新增多语言实现（TypeScript和Python对照）
- 新增s12阶段的高级功能演示
- 完善任务依赖关系管理和循环检测
- 增强错误恢复机制和系统提示词构建
...
## 简介
本教程围绕 Mini-Claude-Code 的十二个学习阶段（s01 到 s12），系统讲解如何逐步构建一个具备工具调用、文件系统操作、**完整任务管理框架**、上下文隔离、技能系统、内存管理与错误恢复能力的智能体。每个阶段在前一阶段基础上扩展功能，形成"可执行、可演示、可迭代"的渐进式学习路径。教程不仅解释核心概念与实现细节，还提供关键函数说明、实际使用示例以及阶段性练习与挑战任务，帮助读者从零到一掌握端到端的工程化实现。

**重要更新**：本次更新反映了应用的重大架构升级，从简单的todo管理扩展为完整的生产级任务管理框架，包括多语言实现（TypeScript和Python对照）、高级功能演示（s10系统提示词构建、s11错误恢复机制、s12高级集成）和完整的依赖关系管理。

## 项目结构
仓库采用基于 SummaryStage 的全新模块化架构，核心入口位于 SummaryStage/src/main.ts，通过 Agent 组装器将各个功能模块有机组合。新增的 s10、s11 和 s12 阶段提供了完整的教育Harness实现，展示多语言对照和高级功能演示。
...
-- 功能模块
feat_task["features/task/index.ts<br/>完整任务管理"]
...
-- s12 教育Harness
s12_ts["s12/index.ts<br/>TypeScript实现"]
s12_py["s12/index.py<br/>Python对照实现"]
root --> readme
root --> summary
root --> src
summary --> main
...
- [src/s12/index.ts:1-600](file://src/s12/index.ts#L1-L600)
...
- [SummaryStage/src/features/task/task-manager.ts:1-351](file://SummaryStage/src/features/task/task-manager.ts#L1-L351)
- [src/s10/index.ts:246-430](file://src/s10/index.ts#L246-L430)
- [src/s11/index.ts:106-140](file://src/s11/index.ts#L106-L140)

## 架构总览
整体架构采用"Agent 组装器 + 模块化功能 + 工具注册表"的三层架构，每层在前一层基础上增加新的抽象与约束，最终形成稳定、可扩展的智能体框架。新增的 s10、s11 和 s12 阶段展示了教育Harness的完整实现，提供多语言对照和高级功能演示。
...
-- s12 教育Harness
advancedInt["Advanced Integration<br/>多语言对照"]
-- 基础工具
bash["bash"]
read_file["read_file"]
...
recovery --> edit_file
-- s12连接
advancedInt --> promptBuilder
advancedInt --> recovery
```
...
### s12：高级集成演示（教育Harness）
- **核心目标**：展示多语言对照和高级功能集成，提供完整的教育Harness实现。
- **关键实现**：
  - TypeScript 和 Python 双语对照实现，便于多语言学习。
  - 集成完整的任务管理、错误恢复、系统提示词构建等功能。
  - 提供丰富的命令行接口和交互体验。
- **学习要点**：
  - 多语言实现的一致性与差异性。
  - 高级功能的集成与协调。
  - 教育Harness的设计理念与实现技巧。
- **实际使用示例**：
  - 运行 TypeScript 版本，观察完整的功能演示。
  - 对照 Python 版本，理解语言间的差异。
  - 使用各种命令行接口探索不同功能。
- **练习与挑战**：
  - 练习：在两个版本间对比功能实现，找出差异点。
  - 挑战：基于 s12 的架构，扩展新的功能模块。

```mermaid
flowchart TD
Start(["开始学习"]) --> MultiLang["多语言对照<br/>TypeScript vs Python"]
MultiLang --> Features["功能集成<br/>任务管理 + 错误恢复 + 提示词构建"]
Features --> Commands["命令行接口<br/>/prompt /sections /memories 等"]
Commands --> Demo["完整演示<br/>教育Harness"]
Demo --> End(["学习完成"])
```
...
**章节来源**
- [src/s12/index.ts:1-600](file://src/s12/index.ts#L1-L600)
- [src/s12/index.py:1-600](file://src/s12/index.py#L1-L600)

### s07：完整任务管理框架
- **核心目标**：从简单的todo管理发展为完整的生产级任务管理框架，支持复杂的依赖关系和状态管理。
- **关键实现**：
  - TaskManager 提供完整的 CRUD 操作和状态管理。
  - 支持任务依赖关系的双向同步和循环检测。
  - 自动解阻塞机制，当任务完成后自动解除依赖。
  - 持久化存储，支持跨会话的任务状态保持。
- **学习要点**：
  - 任务状态机的完整实现。
  - 依赖关系管理与循环检测算法。
...
+create(input) : Task
+get(taskId) : Task
+update(taskId, input) : Task
+list() : Task[]
+getReadyTasks() : Task[]
+getStats() : Stats
+render() : string
-_hasCycle(taskId, blockedBy) : boolean
-_nextId() : number
-_load() : void
-_save() : void
}
...
**图表来源**
- [SummaryStage/src/features/task/task-manager.ts:49-351](file://SummaryStage/src/features/task/task-manager.ts#L49-L351)

**章节来源**
- [SummaryStage/src/features/task/index.ts:1-154](file://SummaryStage/src/features/task/index.ts#L1-L154)
- [SummaryStage/src/features/task/task-manager.ts:1-351](file://SummaryStage/src/features/task/task-manager.ts#L1-L351)
...
- **s12 教育Harness**：提供多语言对照实现，集成所有高级功能。
...
s11_py --> dotenv
-- s12依赖
s12_ts["@anthropic-ai/sdk"] --> s12_ts
s12_ts --> dotenv
s12_py["@anthropic-ai/sdk"] --> s12_py
s12_py --> dotenv
```
...
- [src/s12/package.json:14-22](file://src/s12/package.json#L14-L22)
...
- [src/s12/package.json:1-24](file://src/s12/package.json#L1-L24)
...
- **任务管理优化**：s07 阶段的 TaskManager 采用 Map 数据结构，提供 O(1) 的任务查找和更新。
- **循环检测算法**：使用 DFS 算法进行依赖关系循环检测，时间复杂度为 O(V+E)。
- **持久化存储**：任务状态和记忆数据采用 JSON 序列化，支持快速加载和保存。
- **多语言性能**：s12 阶段的多语言实现保持性能一致性，避免语言特定的性能瓶颈。
...
- [src/s11/index.ts:403-483](file://src/s11/index.ts#L403-L483)
- [SummaryStage/src/features/task/task-manager.ts:296-323](file://SummaryStage/src/features/task/task-manager.ts#L296-L323)

## 结论
通过基于 SummaryStage 的全新模块化架构，我们构建了一个从"基础工具调用"到"完整任务管理框架"，再到"系统提示词构建和错误恢复"，最终扩展到"多语言教育Harness"的完整智能体框架。新增的 s10、s11 和 s12 阶段展示了教育Harness的完整实现，s10 强调了"系统提示词管道化构建"的教学理念，s11 实现了"三层错误恢复机制"的实践案例，s12 提供了"多语言对照和高级集成"的学习体验。

新架构通过 Agent 组装器和模块注册机制，实现了更好的可维护性、扩展性和安全性。特别是 s07 阶段的完整任务管理框架，提供了生产级的任务依赖关系管理、循环检测和自动解阻塞功能，为复杂的多步骤任务执行提供了坚实的基础。s10、s11、s12 阶段进一步增强了系统的鲁棒性和实用性。

每一阶段都围绕一个核心抽象展开，既保持了可理解性，又具备工程化落地价值。建议在掌握各阶段原理后，结合真实场景进行定制化扩展，如引入更多工具、完善错误恢复与审计日志、增强安全防护、实现更复杂的状态机和依赖关系管理等。
...
- **阶段性练习与挑战建议**：
  ...
  - s07：创建包含多个依赖关系的复杂任务图，验证循环检测和自动解阻塞。
  ...
  - s12：对比 TypeScript 和 Python 实现，理解多语言架构的设计理念。
- **参考文件路径**：
  ...
  - s07：[SummaryStage/src/features/task/index.ts](file://SummaryStage/src/features/task/index.ts), [SummaryStage/src/features/task/task-manager.ts](file://SummaryStage/src/features/task/task-manager.ts)
  ...
  - s12：[src/s12/index.ts](file://src/s12/index.ts), [src/s12/index.py](file://src/s12/index.py)